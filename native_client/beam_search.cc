...
public:
  ...
  KenLMBeamScorer(const char *kenlm_model_path)
    : lm_weight(1.0f)
  ...
  void ExpandState(const KenLMBeamState &from_state, int from_label,
    ...
    if (label_to_char[to_label] != ' ') {
      to_state->incomplete_word += label_to_char[to_label];
...
private:
  ...
  static const char label_to_char[];
...
const char KenLMBeamScorer::label_to_char[] = {
  ' ',
  'a',
  'b',
  'c',
